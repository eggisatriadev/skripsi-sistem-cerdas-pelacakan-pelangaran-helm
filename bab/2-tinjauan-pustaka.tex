...
Tabel~\ref{tab:penelitian_terdahulu} merangkum empat penelitian terdahulu yang relevan, mulai dari evaluasi kinerja simpang berbasis kamera omnidirectional sampai pelacakan multiobjek udara berbasis Kafka--Spark. Empat kolom utama yang dibandingkan adalah peneliti dan judul, metodologi, data uji, dan hasil utama; satu kolom tambahan menampilkan celah penelitian masing-masing. Pola yang terbaca cukup jelas: tiga dari empat studi menempati salah satu blok pekerjaan---deteksi saja (\cite{aboah2023helmet}), pelacakan saja (\cite{liu2025vehicle}), atau evaluasi kinerja simpang dengan modul deteksi sebagai pendukung (\cite{alunjati2025intersection})---dan hanya satu (\cite{do2025ramots}) yang menyentuh jalur \textit{stream processing} dari ujung ke ujung. Celah yang muncul berulang adalah ketiadaan rangkaian lengkap deteksi--pelacakan--filter spasial-temporal--distribusi peristiwa--persistensi pada satu sistem yang diuji di lingkungan kampus tropis. Penelitian ini mengisi celah tersebut dengan mengintegrasikan YOLOv8, ByteTrack, filter \textit{Intersection over Area}, konfirmasi temporal \textit{N-Frame}, serta jalur Apache Kafka--Apache Spark Structured Streaming yang berhilir pada \textit{data mart} PostgreSQL.

\vspace{0.5\baselineskip}

{\footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.20\textwidth} >{\RaggedRight\arraybackslash}p{0.20\textwidth} >{\RaggedRight\arraybackslash}p{0.11\textwidth} >{\RaggedRight\arraybackslash}p{0.21\textwidth} >{\RaggedRight\arraybackslash}p{0.20\textwidth}@{}}
    \caption{Ringkasan Penelitian Terdahulu}
    \label{tab:penelitian_terdahulu} \\
    \toprule
    \textbf{Peneliti \& Judul} & \textbf{Metodologi} & \textbf{Data} & \textbf{Hasil Utama} & \textbf{Celah / Posisi terhadap Penelitian Ini} \\
    \midrule
    \endfirsthead
    ...
    Fokus pada pencacahan arus simpang, belum membahas validasi pelanggaran spasial-temporal maupun jalur \textit{event stream} ke \textit{data mart}. \\
    ...
    Belum diuji pada aliran video langsung; tidak ada lapisan distribusi peristiwa untuk konsumsi analitik di hilir. \\
    ...
    Berhenti pada deteksi tingkat \textit{frame}; tidak ada filter spasial IoA, konfirmasi temporal \textit{N-Frame}, atau persistensi ke \textit{data mart}. \\
    ...
    Skenario udara dan tanpa GPU di sisi tepi belum dieksplorasi; penelitian ini menguji jalur Kafka--Spark pada CCTV darat berbasis CPU. \\
...
\item \textit{Value}: Data perlu menghasilkan nilai guna, misalnya dalam bentuk \textit{dashboard} analitik untuk mendukung pengambilan keputusan.
\end{enumerate}

Penelitian ini menghadapi tantangan terbesar pada aspek \textit{volume} dan \textit{velocity}: aliran video CCTV bekerja terus-menerus dengan resolusi tinggi sehingga \textit{batch processing} berbasis siklus pemuatan tidak realistis untuk dipakai sebagai jalur utama. Karena alasan tersebut, \textit{stream processing} menjadi pilihan lebih sesuai, dan salah satu kerangka kerja yang relevan adalah \textit{Kappa Architecture} yang mengonsolidasikan jalur historis dan \textit{real-time} menjadi satu alur data \cite{marz2015big}.
...
\subsubsection{Komponen Galat YOLOv8 untuk Latihan Multikelas}

Latihan YOLOv8 untuk multikelas menggabungkan tiga komponen galat yang masing-masing menargetkan aspek prediksi yang berbeda. Komponen pertama adalah \textit{box loss}, yang mengukur jarak antara kotak pembatas hasil prediksi dan kotak rujukan. Komponen kedua adalah \textit{class loss} yang menilai kebenaran label kelas pada setiap kotak. Komponen ketiga adalah \textit{Distribution Focal Loss} (DFL) yang menjadi penanda khas YOLOv8: alih-alih meregresi nilai tunggal koordinat kotak, DFL memodelkan distribusi diskret atas posisi sub-pixel sehingga model mampu menangkap ketidakpastian koordinat secara lebih halus dan stabil pada objek kecil maupun padat \cite{yaseen2024yolov8indepthexplorationinternal}. Tiga komponen tersebut dipantau secara terpisah pada kurva latihan dan validasi---termasuk yang dilaporkan pada Bab 4---sehingga perilaku konvergensi dapat dianalisis per komponen, bukan hanya total galat.
...
istilah, rumus, dan konteks analitik tetap konsisten pada seluruh \textit{dashboard}.
...
\textit{Extract--Transform--Load} (ETL) sebelum ditampilkan pada \textit{dashboard}.
