...
Setelah pelatihan selesai, kinerja model dinilai pada himpunan validasi (\textit{validation set}) memakai tiga metrik standar deteksi objek: \textit{Mean Average Precision} pada ambang IoU 50\% (mAP@50), \textit{precision}, dan \textit{recall}. Ketiganya saling melengkapi karena mAP memberi gambaran kualitas pada beragam ambang \textit{confidence}, sedangkan \textit{precision} dan \textit{recall} menunjukkan keseimbangan antara deteksi yang benar dan deteksi yang terlewat. Ringkasan metrik agregat dirangkum pada Tabel~\ref{tab:metrik-validasi}.
...
% --- ANALITIK 1 ---
Pada Tabel~\ref{tab:metrik-validasi}, \textit{precision} 87,5\% dan \textit{recall} 86,5\% hanya berselisih 1,0 poin. Selisih sekecil itu menandakan model tidak terlalu agresif memunculkan deteksi yang ujungnya salah, namun juga tidak terlalu hati-hati sampai melewatkan objek yang seharusnya tertangkap. Skor F1, yang merupakan rata-rata harmonik antara \textit{precision} dan \textit{recall}, juga ada di kisaran 87,0\%; angka ini memperkuat kesan bahwa keseimbangan kedua metrik bukan kebetulan dari salah satu sisi saja. Pembatasan inferensi pada \textit{Region of Interest} (ROI) ikut menjaga \textit{precision} tetap di atas 85\%. Tanpa ROI, model akan ikut menanggapi tekstur latar di trotoar atau jalur berlawanan dan melahirkan banyak deteksi positif palsu (\textit{false positives}). Pada skenario \textit{streaming} di gerbang Itera, ambang \textit{precision} 85\% perlu dijaga karena setiap deteksi salah berpotensi memicu rangkaian pencatatan dan kirim pesan yang sebenarnya tidak perlu.

Nilai mAP@50 sebesar 91,4\% sebanding dengan kinerja sistem deteksi pelanggaran helm berbasis YOLOv8 pada skenario waktu nyata yang dilaporkan oleh \cite{aboah2023helmet}. Meski begitu, \textit{precision} dan \textit{recall} yang masih di bawah 90\% menunjukkan model belum sempurna terutama pada objek kecil dan fitur yang ambigu. Tantangan paling jelas berasal dari beda ukuran \textit{bounding box} antara badan sepeda motor dan area kepala pengendara: kepala hanya menempati sebagian kecil piksel di citra, dan saat motor menjauh dari kamera tepi gerbang, informasi visualnya makin terbatas. Pada kondisi seperti itu, model lebih rentan salah membedakan pengendara berhelm, pengendara bertopi, atau pengendara berambut tebal. Walau begitu, \textit{recall} 86,5\% berarti hanya sekitar 13,5\% pelanggaran yang lolos deteksi (\textit{false negatives}) pada himpunan validasi---angka yang masih dapat ditekan lebih jauh oleh filter spasial IoA dan konfirmasi temporal \textit{N-frame} pada tahap berikutnya dalam \textit{pipeline}.

Untuk mengukur seberapa baik model menggeneralisasi pada citra yang belum pernah dilihat, evaluasi dilanjutkan ke himpunan uji (\textit{test set}) yang sepenuhnya terpisah dari proses pelatihan. Hasil per kelas berupa \textit{Average Precision} (AP) pada ambang IoU 50\% disajikan di Tabel~\ref{tab:metrik-test-objek}.
...
% --- ANALITIK 2 ---
Tabel~\ref{tab:metrik-test-objek} memperlihatkan dua hal sekaligus. Pertama, kinerja agregat pada himpunan uji (mAP@50 = 90,0\%) hanya berbeda 1,4 poin dari himpunan validasi (mAP@50 = 91,4\%). Selisih sekecil ini menandakan model tidak \textit{overfit} pada data pelatihan dan masih bisa diandalkan ketika menghadapi citra baru. Kedua, AP per kelas mengikuti urutan yang konsisten dengan ukuran objek di citra: \texttt{MOTORCYCLE} mencatat AP tertinggi 94,0\% karena badan motor menempati piksel dominan dan bentuknya relatif konsisten antar-citra, sedangkan dua kelas atribut kepala punya AP lebih rendah. Selisih 7,0 poin antara \texttt{MOTORCYCLE} dan \texttt{DRIVER\_HELMET} menegaskan tantangan model bukan terletak pada deteksi objek besar, melainkan pada klasifikasi atribut berukuran kecil. Pola seperti ini sejalan dengan isu \textit{scale imbalance} pada deteksi objek kecil yang dilaporkan oleh \cite{oksuz2020imbalance}.

Hasil yang menarik justru muncul pada perbandingan dua kelas pelanggaran. Model lebih presisi mendeteksi \texttt{DRIVER\_NO\_HELMET} (90,0\%) dibanding \texttt{DRIVER\_HELMET} (87,0\%) walaupun jumlah anotasi keduanya nyaris setara (730 berbanding 718). Selisih 3,0 poin ini mengisyaratkan bahwa fitur visual ``tanpa helm''---siluet kepala telanjang, rambut, dan kulit---lebih konsisten dipelajari daripada variasi tampilan helm di lapangan. Helm di area gerbang muncul dalam beragam bentuk, warna, dan ukuran. Variasi itu masih ditambah pantulan cahaya pagi atau sore yang kerap menutupi sebagian permukaan helm dan oklusi parsial oleh kaca depan, penumpang, atau tas. Akibatnya, distribusi fitur ``berhelm'' terlihat lebih heterogen dan model lebih sering salah klasifikasi (\textit{misclassification}). Untungnya, kelas yang paling kritis untuk sistem ini adalah \texttt{DRIVER\_NO\_HELMET}; kelas itu justru memperoleh AP yang lebih tinggi sehingga sasaran utama sistem, yaitu menemukan pelanggaran, tetap tertangani dengan baik.

Secara keseluruhan, mAP@50 sebesar 90,0\% pada himpunan uji menunjukkan modul YOLOv8 cukup matang untuk diintegrasikan ke \textit{pipeline} \textit{streaming} bersama ByteTrack pada purwarupa di Itera. Selisih kinerja antarkelas memang masih ada, terutama pada \texttt{DRIVER\_HELMET}; pengembangan lanjutan dapat difokuskan pada perluasan variasi citra helm, misalnya pengambilan sampel di jam dengan kondisi cahaya berbeda atau pada cuaca berbeda, supaya selisih AP antarobjek menyempit pada iterasi berikutnya.
