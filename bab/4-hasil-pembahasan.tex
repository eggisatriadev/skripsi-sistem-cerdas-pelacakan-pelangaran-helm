...
Selain distribusi kuantitatif, kualitas dataset juga ditinjau dari posisi kemunculan objek pada bidang pandang kamera. Gambar~\ref{fig:heatmap-per-class} menyajikan \textit{annotation heatmap} dengan gradasi biru muda hingga merah pekat sebagai indikator intensitas \textit{bounding box} per \textit{grid} citra. Berdasarkan legenda peta panas, nilai minimum, kuartil pertama, dan median \textit{grid} sama-sama bernilai 0 anotasi; kuartil ketiga sebanyak 11 anotasi; sedangkan \textit{grid} terpadat memuat 172 anotasi.
...
Gambar~\ref{fig:heatmap-per-class}(a) menampilkan kepadatan kelas \texttt{DRIVER\_HELMET} yang terpusat di bagian tengah hingga kanan atas citra dengan inti merah pada lajur masuk utama. Gambar~\ref{fig:heatmap-per-class}(b) memperlihatkan kepadatan kelas \texttt{DRIVER\_NO\_HELMET} yang berpola serupa, namun titik terpadatnya sedikit melebar ke arah kiri tengah, sehingga mengindikasikan kebiasaan sebagian pengendara mengambil sisi tertentu pada lajur masuk. Gambar~\ref{fig:heatmap-per-class}(c) menggambarkan kelas \texttt{MOTORCYCLE} dengan zona kepadatan yang hampir berimpit dengan area pengendara berhelm karena badan motor selalu hadir sebagai konteks anotasi keduanya. Berangkat dari kesamaan zona kepadatan tersebut, \textit{Region of Interest} (ROI) ditetapkan menyusur kontur jalan yang menjadi lintasan utama kendaraan, kira-kira mencakup bagian tengah hingga kanan atas citra.

Area di luar ROI didominasi warna biru muda hingga abu-abu yang berarti \textit{grid} pada wilayah tersebut bernilai 0 anotasi atau mendekati nol. Wilayah itu mencakup latar pasif seperti langit di bagian atas, trotoar, dan jalur berlawanan di tepi citra; kendaraan maupun pengendara tidak melintas pada area tersebut sehingga tidak ada \textit{bounding box} yang dihasilkan. Pembatasan ROI memangkas pemrosesan area latar yang nihil anotasi, sehingga inferensi YOLOv8 hanya berjalan pada zona berkepadatan tinggi. Selain menurunkan beban komputasi, pembatasan ini juga menaikkan \textit{precision} sekaligus menekan \textit{false positive} pada inferensi waktu nyata.
