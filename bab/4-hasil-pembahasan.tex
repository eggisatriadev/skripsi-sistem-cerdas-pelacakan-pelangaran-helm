...
Subbab ini membahas tiga pilar transformasi data di hilir \textit{pipeline} deteksi. Pertama, implementasi gudang analitik berbasis skema bintang (\textit{star schema}) pada PostgreSQL yang menampung 2.301.002 baris kejadian pelanggaran sepanjang tahun 2025 dari tiga kamera operasional. Kedua, evaluasi kinerja kueri (\textit{query benchmark}) yang membuktikan strategi indeks dan denormalisasi temporal mampu memenuhi kebutuhan dasbor \textit{near real-time}. Ketiga, perancangan antarmuka dasbor interaktif berbasis Grafana lengkap dengan pembacaan pola operasional dari rekaman 10 jam pengamatan lapangan, sebagai bukti peran sistem sebagai Sistem Pendukung Keputusan (\textit{Decision Support System}) dalam manajemen keselamatan kampus.
...
Berbeda dengan pengujian \textit{pipeline} pada Sub-bab~\ref{subsec:persistensi-filter} yang memakai potongan 3.001 \textit{frame}, bagian ini melaporkan operasi sistem pada rekaman penuh. Sumber data berasal dari kamera \texttt{gate\_utama\_01} di gerbang utama Itera, direkam selama 10 jam pada hari kerja pukul 07.00--17.00 WIB. Pada rentang waktu tersebut, \textit{pipeline} memproses 11.172 deteksi mentah dari ketiga kelas dan menyalurkannya lewat rangkaian filter spasial-temporal yang sama dengan pengujian sebelumnya. Setelah seleksi kelas, filter IoA, konfirmasi N-\textit{frame}, dan \textit{dedup} per-\textit{track} berjalan, \textit{data mart} menerima 113 insiden pelanggaran berkendara tanpa helm secara persisten. Tiap baris terhubung ke dimensi waktu, dimensi kamera, dan dimensi jenis pelanggaran.

Tabel~\ref{tab:datamart-sample} menyajikan contoh tiga baris hasil \textit{join} \texttt{fact\_violations} dengan tabel dimensi. Setiap baris menggabungkan waktu kejadian, lokasi sensor, jenis pelanggaran, probabilitas deteksi, dan latensi pemrosesan dalam satu tampilan kueri. Baris pertama tercatat pada periode pagi dengan probabilitas 85,2\% dan latensi 45,2 milidetik. Baris kedua bergeser ke periode sore dengan probabilitas 65,4\% dan latensi 92,5 milidetik. Baris ketiga kembali ke profil lebih stabil dengan probabilitas 82,3\% dan latensi 41,0 milidetik. Contoh ini memperlihatkan bahwa struktur skema bintang tidak hanya menghasilkan total pelanggaran, tetapi juga mendukung pembacaan hubungan antara waktu, lokasi, kualitas deteksi, dan beban pemrosesan.

Dari 113 insiden yang termuat, 12 di antaranya membawa tautan ke berkas gambar bukti sehingga rekaman pelanggaran dapat diaudit ulang oleh operator gerbang. Struktur ini mendukung kueri OLAP yang lazim, seperti total pelanggaran per jam, rata-rata \textit{confidence} per periode harian, distribusi kamera, dan tabulasi silang antara jam dan hari. Dengan susunan tersebut, \textit{data mart} memenuhi fungsi ganda: mempertahankan detail kejadian untuk audit dan menyediakan agregasi yang siap dikonsumsi oleh panel \textit{dashboard}.
...
Implementasi \textit{data mart} pada penelitian ini dibangun menggunakan skema bintang (\textit{star schema}) karena pola tersebut memisahkan data kuantitatif pada tabel fakta dan atribut deskriptif pada tabel dimensi. Dalam pendekatan Kimball, perancangan dimensional dimulai dari penentuan proses bisnis, deklarasi \textit{grain}, pemilihan dimensi, kemudian penetapan fakta atau \textit{measure} yang konsisten dengan \textit{grain} tersebut~\cite{kimball2013dwtoolkit,kimball_grain}. Proses bisnis yang dimodelkan pada penelitian ini adalah pencatatan kejadian pelanggaran helm yang telah lolos deteksi YOLOv8, pelacakan ByteTrack, filter spasial, dan validasi temporal. Oleh karena itu, tabel \texttt{fact\_violations} ditempatkan sebagai pusat skema, sedangkan \texttt{dim\_camera}, \texttt{dim\_violation\_type}, dan \texttt{dim\_time} digunakan sebagai konteks analitik. Susunan ini membuat kueri analitik dapat menjumlahkan metrik pelanggaran berdasarkan waktu, lokasi kamera, dan tipe pelanggaran tanpa harus membaca ulang struktur data operasional dari sistem deteksi.

\subsubsection{Penetapan \textit{grain} tabel fakta}

\textit{Grain} adalah keputusan paling penting dalam rancangan skema bintang karena menjelaskan makna satu baris pada tabel fakta. Kimball menekankan bahwa deklarasi \textit{grain} menjadi kontrak desain: setiap dimensi dan fakta yang dimasukkan harus konsisten dengan makna satu baris tersebut~\cite{kimball_grain}. Pada sistem ini, \textit{grain} \texttt{fact\_violations} ditetapkan sebagai berikut.
...
Deklarasi tersebut membedakan data mart dari log deteksi mentah per \textit{frame}. Pada tahap hulu, satu pengendara yang melintas dapat terdeteksi berulang selama beberapa \textit{frame}. Jika semua deteksi mentah langsung dimasukkan ke \textit{data mart}, angka yang muncul pada \textit{dashboard} akan merepresentasikan jumlah \textit{bounding box} atau jumlah \textit{frame}, bukan jumlah pelanggaran. Oleh karena itu, filter spasial--temporal dan mekanisme \textit{deduplication} berdasarkan \texttt{track\_id} dilakukan sebelum data masuk ke tabel fakta. Hasilnya, fungsi agregasi seperti \texttt{COUNT(*)} atau \texttt{SUM(violation\_count)} memiliki makna bisnis yang stabil, yaitu jumlah kejadian pelanggaran.

Pemilihan \textit{grain} per-kejadian juga sesuai dengan konsep \textit{transaction fact table}. Kimball menjelaskan bahwa satu baris pada \textit{transaction fact table} berhubungan dengan satu peristiwa pengukuran pada titik ruang dan waktu tertentu; tabel ini bersifat ekspresif karena memungkinkan analisis rinci dan agregasi ulang ke berbagai sudut pandang~\cite{kimball_transaction_fact}. Untuk data CCTV, karakter ini penting karena setiap kejadian masih perlu ditelusuri kembali melalui \texttt{event\_id}, \texttt{track\_id}, waktu kejadian, \textit{confidence}, \textit{bounding box}, latensi, dan tautan gambar bukti. Dengan demikian, penyimpanan atomik per-kejadian tidak bertentangan dengan kebutuhan OLAP. Data atomik dipertahankan sebagai sumber kebenaran, sedangkan ringkasan per-jam dibuat melalui \textit{view} atau tabel agregat untuk mempercepat visualisasi.

\subsubsection{Struktur skema bintang dan peran tabel}

Gambar~\ref{fig:stardesign} memperlihatkan bentuk skema bintang yang digunakan. Tabel \texttt{fact\_violations} berisi \textit{foreign key} menuju tiga dimensi utama, yaitu \texttt{timestamp\_fk}, \texttt{camera\_fk}, dan \texttt{violation\_type\_fk}. Selain itu, tabel fakta menyimpan identitas kejadian seperti \texttt{event\_id} dan \texttt{track\_id}, atribut bukti seperti koordinat \textit{bounding box} dan \texttt{evidence\_image\_url}, serta metrik analitik seperti \texttt{violation\_count}, \texttt{confidence}, dan \texttt{processing\_latency\_ms}. Pemisahan ini membuat tabel fakta tetap berorientasi pada kejadian dan pengukuran, sementara atribut deskriptif yang dipakai untuk penyaringan atau pengelompokan ditempatkan pada tabel dimensi.

Dimensi \texttt{dim\_camera} menyimpan konteks sensor, yaitu kode kamera, nama kamera, deskripsi lokasi, koordinat GPS, dan status kamera. Dimensi ini memungkinkan analisis distribusi pelanggaran berdasarkan titik pengamatan tanpa menggandakan data lokasi pada setiap baris fakta. Dimensi \texttt{dim\_violation\_type} menyimpan kode, nama, deskripsi, dan tingkat keparahan pelanggaran. Walaupun penelitian ini berfokus pada pelanggaran tanpa helm, struktur tersebut membuat sistem dapat diperluas ke tipe pelanggaran lain dengan menambah baris dimensi, bukan mengubah struktur tabel fakta. Dimensi \texttt{dim\_time} menyimpan atribut waktu seperti tanggal, tahun, bulan, hari, jam, periode harian, akhir pekan, dan periode ujian. Atribut temporal yang sering dibaca oleh \textit{dashboard}, seperti \texttt{date}, \texttt{hour}, \texttt{minute}, \texttt{day\_of\_week}, dan \texttt{time\_period}, juga disalin ke tabel fakta sebagai bentuk denormalisasi terbatas untuk mempercepat kueri operasional.

Denormalisasi temporal tersebut merupakan keputusan praktis. Secara konseptual, dimensi waktu sebaiknya tidak dibiarkan membesar tanpa kendali sampai setiap milidetik menjadi baris dimensi. Pada pengembangan berikutnya, rancangan waktu dapat diperhalus menjadi \texttt{dim\_date} dengan \textit{grain} satu tanggal, \texttt{dim\_time\_of\_day} dengan \textit{grain} satu menit atau satu detik dalam sehari, serta \texttt{event\_timestamp} yang tetap disimpan pada tabel fakta untuk audit. Namun, pada implementasi ini, atribut waktu yang sudah diturunkan di tahap ETL membantu \textit{dashboard} memperoleh tren per-jam dan periode harian tanpa menjalankan fungsi ekstraksi waktu secara berulang pada setiap penyegaran panel.

\subsubsection{Klasifikasi \textit{measure} pada tabel fakta}

Menurut Kimball, fakta numerik pada tabel fakta dapat diklasifikasikan menjadi \textit{additive}, \textit{semi-additive}, dan \textit{non-additive facts}~\cite{kimball_additive_facts}. Klasifikasi ini penting karena menentukan fungsi agregasi yang benar. Tabel~\ref{tab:klasifikasi-measure-dw} merangkum jenis \textit{measure} yang digunakan pada \texttt{fact\_violations}.
...
Berdasarkan Tabel~\ref{tab:klasifikasi-measure-dw}, \texttt{violation\_count} merupakan \textit{measure} utama yang bersifat \textit{fully additive}. Karena \textit{grain} tabel fakta adalah satu kejadian pelanggaran, nilai \texttt{violation\_count} diisi 1 pada setiap baris. Total pelanggaran kemudian diperoleh dengan menjumlahkan kolom tersebut berdasarkan dimensi waktu, kamera, atau tipe pelanggaran. Secara matematis, \texttt{SUM(violation\_count)} akan setara dengan \texttt{COUNT(*)} selama satu baris fakta tetap merepresentasikan satu kejadian. Akan tetapi, penggunaan \texttt{violation\_count} tetap dipertahankan karena lebih eksplisit sebagai metrik bisnis dan memudahkan pembentukan tabel agregat.

Sebaliknya, \texttt{confidence} dan \texttt{processing\_latency\_ms} tidak diperlakukan sebagai \textit{additive measure}. Penjumlahan nilai \textit{confidence} tidak memiliki makna analitik karena angka tersebut merepresentasikan tingkat keyakinan model pada satu kejadian. Oleh karena itu, kueri \textit{dashboard} menggunakan rata-rata, nilai minimum, dan nilai maksimum untuk menilai stabilitas model. Prinsip yang sama berlaku pada latensi: rata-rata latensi menunjukkan beban komputasi normal, sedangkan nilai maksimum dapat menandai lonjakan beban atau keterlambatan pemrosesan.


\subsection{Evaluasi Kinerja Kueri \textit{Data Mart}}
\label{subsec:evaluasi-kueri-datamart}

Skema bintang yang telah dirinci pada Sub-bab~\ref{subsec:implementasi-star-schema} perlu dibuktikan tidak hanya rapi secara konseptual, melainkan juga responsif ketika dieksekusi pada volume produksi. Pengujian dilakukan dengan \texttt{EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)} di PostgreSQL terhadap \texttt{fact\_violations} yang berisi 2.301.002 baris kejadian pelanggaran sepanjang tahun 2025 dari tiga kamera aktif. Sebanyak 22 kueri \textit{benchmark} dijalankan untuk mencakup tiga peran utama yang dilayani \textit{data mart}: pemantauan harian dasbor, agregasi periodik, dan ringkasan statistik tahunan. Target kinerja dirumuskan sederhana: kueri yang melayani panel dasbor diharapkan selesai di bawah ambang 50 ms supaya interval penyegaran 10 detik tetap mulus, sedangkan kueri agregat periodik dapat ditoleransi pada rentang ratusan milidetik.
...
\midrule
\multicolumn{3}{l}{\textit{Tier} 1 (cepat, $<$50 ms)\,---\,panel \textit{real-time} dasbor} \\
\midrule
