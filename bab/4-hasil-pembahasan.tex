...
Selisih angka antara pengujian \textit{pipeline} (9.544 $\to$ 2) dan operasi 10 jam (11.172 $\to$ 113) bukan tanda filter berperilaku berbeda, melainkan konsekuensi langsung dari tahap \textit{dedup} per-\textit{track} yang menyesuaikan diri dengan jumlah pengendara unik pada rentang rekaman. Tiga tahap awal---seleksi kelas, filter IoA, dan konfirmasi N-\textit{frame}---memangkas dengan rasio yang konsisten karena keputusan filter dievaluasi pada tiap kandidat secara independen tanpa memandang panjang rekaman. Yang berbeda adalah ekor jalur pelacakan. Snippet 3.001 \textit{frame} hanya memuat dua pengendara tanpa helm yang muncul berulang pada \textit{frame} berturut-turut, sehingga 25 konfirmasi tingkat-\textit{frame} dipadatkan menjadi dua \textit{Track ID}---rasio kompresi 12,5$\times$. Rekaman 10 jam, sebaliknya, mengamati 113 pengendara berlainan yang masing-masing hanya melintas beberapa detik di area pantau, sehingga \textit{dedup} mengompresi konfirmasi tingkat-\textit{frame} ke \textit{Track ID} unik dengan rasio yang jauh lebih kecil per pengendara. Angka 113 dengan demikian merepresentasikan jumlah pelanggar fisik yang tertangkap sepanjang 10 jam pengamatan---bukan jumlah \textit{frame} berisi pelanggaran---sehingga konsisten dengan rangkaian filter yang sama yang dipakai pada pengujian \textit{pipeline}.
