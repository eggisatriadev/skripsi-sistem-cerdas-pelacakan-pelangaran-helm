...
Akumulasi efek dari rangkaian filter ini terasa nyata bagi operator gerbang yang akan memakai dashboard. Tanpa filter dan \textit{dedup} per-\textit{track}, satu pengendara tanpa helm yang melintas selama 5 detik pada 15 FPS dapat tercatat hingga 75 kali. Dengan susunan filter saat ini, pengendara yang sama tercatat satu kali saja. Artinya keluaran sisi tepi sudah berbentuk \textit{event} tervalidasi --- bukan deteksi mentah --- yang layak menjadi muatan aliran ke lapisan distribusi di sub-bab berikutnya.
...
\subsection{Validasi \textit{End-to-End} dengan Pencocokan Manual}
\label{subsec:validasi-e2e}
 
% =============================================================
% CATATAN UNTUK PENULIS:
% Angka yang masih ditulis sebagai [XX] atau [XX,X] adalah placeholder.
% Isi setelah:
%   1. Anotasi manual rekaman 5 Februari 2026 (14.00-15.59 WIB) selesai
%   2. Pipeline dijalankan pada rekaman yang sama
%   3. Skrip tools/match_e2e.py dijalankan untuk mendapatkan TP/FP/FN
% =============================================================
 
Validasi \textit{end-to-end} dilakukan dengan membandingkan jumlah pelanggar yang dicatat \textit{pipeline} terhadap perhitungan manual oleh pengamat manusia, sebagaimana praktik evaluasi pada penelitian \textit{video analytics} lapangan. Sumber data adalah rekaman CCTV gerbang utama Itera pada tanggal 5 Februari 2026 pukul 14.00.00 sampai 15.59.59 WIB, yaitu segmen \textit{peak hour} 2 jam yang dipilih karena memuat kepadatan tertinggi pada jam pergantian kuliah sore. Anotasi manual dilakukan oleh penulis dengan memutar rekaman pada kecepatan $1{,}5\times$ dan mencatat tiap pelanggar pada \textit{spreadsheet} berisi kolom \textit{timestamp} masuk-keluar dan arah gerak; pengulangan menonton dilakukan pada hari berbeda untuk menekan bias pengamat. Satu pelanggar manual dipasangkan satu-ke-satu dengan satu \textit{event pipeline} bila \textit{timestamp event} masuk dalam jendela toleransi $\pm 5$ detik di sekitar masa kehadiran pelanggar di area pantau. Toleransi ini cukup untuk menyerap jeda konfirmasi N-\textit{frame} (0,16 detik pada 18,9 FPS) dan latensi distribusi Kafka--Spark--PostgreSQL. Pelanggar yang sama yang masuk-keluar lebih dari 60 detik dihitung dua entitas terpisah, simetris dengan perilaku \texttt{Track ID} ByteTrack yang me-reset identitas setelah \textit{lost track buffer}, sehingga pembandingannya adil.
 
Hasil pencocokan dan metrik kinerja tersaji di Tabel~\ref{tab:e2e-konfusi-metrik}. Selama 2 jam pengamatan, pengamat manusia mendokumentasikan \textbf{[N\_manual]} pelanggar, sementara \textit{pipeline} mencatat \textbf{[N\_pipeline]} \textit{event}. Dari jumlah itu, \textbf{[TP]} \textit{event} cocok kedua sumber (\textit{True Positive}), \textbf{[FP]} \textit{event pipeline} tidak menemukan pasangan manual (\textit{False Positive}), dan \textbf{[FN]} pelanggar manual lolos dari sistem (\textit{False Negative}). Mayoritas \textit{False Positive} berasal dari \textbf{[sebutkan: oklusi parsial, pantulan helm pengendara di kaca mobil, dsb.]}, sedangkan \textit{False Negative} dijumpai pada \textbf{[sebutkan: pelanggar tertutup penuh kendaraan lain, kondisi \textit{backlight} sore, dsb.]}. Dengan demikian, \textit{precision} \textbf{[XX,X]\%} menunjukkan kelayakan alarm yang dihasilkan, \textit{recall} \textbf{[XX,X]\%} menunjukkan kelengkapan deteksi terhadap kejadian nyata, dan F1-Score \textbf{[XX,X]\%} merangkum keseimbangan keduanya. \textit{Counting accuracy} pada total \textbf{[XX,X]\%} memperlihatkan jumlah agregat \textit{pipeline} hampir setara hitungan manusia, sehingga keluaran sistem layak menjadi rujukan operator gerbang.
 
\begin{table}[H]
\centering
\caption{Konfusi pencocokan \textit{event} dan metrik kinerja \textit{end-to-end} pada segmen \textit{peak hour} 5 Februari 2026 pukul 14.00--15.59 WIB.}
\label{tab:e2e-konfusi-metrik}
\begin{tabular}{lc}
\toprule
\textbf{Komponen} & \textbf{Nilai} \\
\midrule
\multicolumn{2}{l}{\textit{Konfusi Event}} \\
\quad \textit{True Positive} (TP): cocok manual \& \textit{pipeline} & \textbf{[TP]} \\
\quad \textit{False Positive} (FP): hanya tercatat \textit{pipeline}   & \textbf{[FP]} \\
\quad \textit{False Negative} (FN): hanya tercatat manual              & \textbf{[FN]} \\
\quad Total pelanggar manual ($N_{\text{manual}} = \text{TP}+\text{FN}$)   & \textbf{[N\_manual]} \\
\quad Total \textit{event pipeline} ($N_{\text{pipeline}} = \text{TP}+\text{FP}$) & \textbf{[N\_pipeline]} \\
\midrule
\multicolumn{2}{l}{\textit{Metrik Kinerja}} \\
\quad \textit{Precision}              & \textbf{[XX,X]\%} \\
\quad \textit{Recall}                 & \textbf{[XX,X]\%} \\
\quad F1-Score                        & \textbf{[XX,X]\%} \\
\quad \textit{Counting Accuracy}      & \textbf{[XX,X]\%} \\
\quad MAPE (per jendela 15 menit)     & \textbf{[XX,X]\%} \\
\bottomrule
\end{tabular}
\end{table}
 
Distribusi temporal pelanggaran pada delapan jendela 15 menit dirangkum di Tabel~\ref{tab:e2e-breakdown-15menit}. Pola yang muncul: penyimpangan terbesar terjadi pada jendela \textbf{[sebutkan jendela]} dengan selisih \textbf{[N]} \textit{event}, sedangkan jendela \textbf{[sebutkan jendela]} mencatat angka manual dan \textit{pipeline} yang nyaris identik. Penyimpangan pada jendela padat dapat dijelaskan oleh meningkatnya frekuensi oklusi antar-kendaraan, sedangkan kekeliruan kecil pada jendela jarang umumnya berasal dari pelanggar yang lewat di tepi area pantau. MAPE \textbf{[XX,X]\%} tetap di kisaran yang dilaporkan literatur \textit{video analytics} sejenis untuk pengamatan satu segmen \textit{peak}, dan menegaskan bahwa laporan jam-per-jam pada dashboard Grafana di Sub-bab~\ref{subsec:grafana} dapat dipercaya untuk pengambilan keputusan operasional. Catatan keterbatasan yang perlu disampaikan terbuka: angka di atas berlaku untuk satu segmen sore cerah; generalisasi ke periode malam, hujan, dan akhir pekan masih perlu pengukuran ulang dan dimasukkan ke agenda pengembangan lanjutan.
 
\begin{table}[H]
\centering
\caption{Distribusi pelanggaran helm manual vs \textit{pipeline} pada delapan jendela 15 menit di segmen 5 Februari 2026 (14.00--15.59 WIB).}
\label{tab:e2e-breakdown-15menit}
\begin{tabular}{lccc}
\toprule
\textbf{Jendela waktu (WIB)} & $N_{\text{manual}}$ & $N_{\text{pipeline}}$ & \textbf{Selisih} \\
\midrule
14.00 -- 14.14 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
14.15 -- 14.29 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
14.30 -- 14.44 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
14.45 -- 14.59 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
15.00 -- 15.14 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
15.15 -- 15.29 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
15.30 -- 15.44 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
15.45 -- 15.59 & \textbf{[..]} & \textbf{[..]} & \textbf{[..]} \\
\midrule
\textbf{Total}  & \textbf{[N\_manual]} & \textbf{[N\_pipeline]} & \textbf{[$\pm$..]} \\
\bottomrule
\end{tabular}
\end{table}
...
Bab 1 menanyakan bagaimana skema \textit{data mart} berbasis \textit{star schema} dan dashboard analitik dirancang untuk mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan (\textit{Decision Support System}) yang memetakan pola spasiotemporal pelanggaran di Itera. Sub-bab ini menjawab pertanyaan tersebut dengan urutan kerja yang paralel dengan Tahap 4 metodologi pada Bab 3, yaitu implementasi \textit{star schema} di PostgreSQL, visualisasi dashboard Grafana, dan analisis wawasan lapangan atas rekaman operasional 10 jam di gerbang utama Itera. Pembahasan dibagi menjadi empat bagian. Tiga bagian pertama mengikuti urutan kerja lapisan hilir, dan bagian terakhir merangkai keempat hasil menjadi jawaban atas Rumusan Masalah ke-3.
% ------------------------------------------------------------
...
\end{table}
\subsection{Visualisasi dashboard Analitik Waktu Nyata (\textit{Real-Time Dashboard})}
\label{subsec:visualisasi-dashboard}

Lapisan presentasi diwujudkan sebagai dashboard interaktif berbasis Grafana yang terhubung langsung ke \texttt{fact\_violations}. Grafana dipilih karena bersifat \textit{open source}, ringan dipasang di server kampus, dan mendukung penyegaran data tiap beberapa detik tanpa konfigurasi tambahan. Tampilan utama dashboard pada Gambar~\ref{fig:grafana-kpi-overview} membagi panel ke dalam empat kategori sesuai kebutuhan pengawasan: indikator cepat, tren waktu, performa sistem AI, dan distribusi spasio-temporal.

% --- EVIDENCE 1: Screenshot Grafana Overview ---
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{gambar/grafana_kpi_overview.png}
\caption{Antarmuka utama dashboard Grafana yang memvisualisasikan data operasional riil selama 10 jam. Sistem merender Indikator Kinerja Utama (KPI) serta dinamika pelanggaran per 15 menit berdasarkan ekstraksi CCTV.}
\label{fig:grafana-kpi-overview}
\end{figure}

Empat kategori panel disusun supaya operator dapat membaca dashboard dari yang paling ringkas ke yang paling rinci. Kategori KPI berada paling atas untuk memberi ringkasan satu pandang, kategori tren waktu mengikuti agar perubahan harian terlihat, kategori performa AI menempati posisi tengah untuk memantau kesehatan model, dan kategori distribusi spasio-temporal di bawah memberi peta panas serta peringkat lokasi rawan. Rincian fungsi tiap kategori ada pada Tabel~\ref{tab:dashboard-panels}.


Pengujian antarmuka mencatat setiap pembaruan di \texttt{fact\_violations} muncul di panel Grafana dalam beberapa detik. Dengan sinkronisasi yang konsisten, dashboard siap dipakai operator gerbang untuk pemantauan harian. Pembacaan pola temporal dan spasial atas data dashboard disajikan pada Sub-bab~\ref{subsec:analisis-data-lapangan}.

% --- EVIDENCE 2: Tabel Konfigurasi Panel ---
\begin{table}[htbp]
\centering
\caption{Daftar segmentasi panel pada dashboard analitik berdasarkan rancangan fungsional sistem.}
...
% ============================================================
\subsection{Analisis Wawasan Lapangan Berdasarkan Visualisasi dashboard}
\label{subsec:analisis-data-lapangan}
% ============================================================

Implementasi \textit{Data Mart} mengekstraksi wawasan operasional dari data lapangan secara berulang. Data yang dipakai adalah hasil inferensi YOLOv8 dari aliran CCTV Kamera Gerbang Utama 1, bukan data sintetis. Pengamatan berlangsung selama 10 jam operasional pada hari kerja (07.00--17.00 WIB). Pembahasan berikut menguraikan keluaran tiap panel dashboard untuk menilai peran sistem sebagai pendukung pengambilan keputusan.
...
% Tiga sub-bab di atas menghasilkan empat bukti yang saling melengkapi. Skema bintang dengan tabel fakta \texttt{fact\_violations} dan tiga tabel dimensi (\texttt{dim\_camera}, \texttt{dim\_violation\_type}, \texttt{dim\_time}) terbukti mampu memuat 113 insiden pelanggaran dari rekaman 10 jam operasional di gerbang utama Itera secara persisten dan siap untuk kueri agregasi. dashboard Grafana memvisualisasikan keempat sudut pandang yang dibutuhkan pengelola: tren agregat harian, distribusi 15 menit, akumulasi jam, dan matriks temporal kepadatan; keempatnya dirakit dari sumber data yang sama tanpa duplikasi. Analisis wawasan lapangan menonjolkan pola spasiotemporal nyata, yaitu distribusi bimodal pagi--sore dengan puncak absolut pukul 14.45--15.15 WIB yang berkorelasi dengan transisi jadwal kelas dan kondisi pasca-hujan. Stabilitas \textit{Avg Confidence} 79,9\% dan \textit{Avg Latency} 49 milidetik selama lonjakan beban memperlihatkan dashboard masih dapat dipercaya saat data mengalir deras. Dengan keempat bukti tersebut, pertanyaan Rumusan Masalah ke-3 sudah memperoleh jawabannya, yaitu skema \textit{data mart} berbasis \textit{star schema} dan dashboard analitik Grafana berhasil mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan yang memetakan pola spasiotemporal pelanggaran di kampus Itera secara terukur dan dapat ditindaklanjuti.
