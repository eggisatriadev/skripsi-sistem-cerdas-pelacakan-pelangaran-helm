...
Akumulasi efek dari rangkaian filter ini terasa nyata bagi operator gerbang yang akan memakai \textit{dashboard}. Tanpa filter dan \textit{dedup} per-\textit{track}, satu pengendara tanpa helm yang melintas selama 5 detik pada 15 FPS dapat tercatat hingga 75 kali. Dengan susunan filter saat ini, pengendara yang sama tercatat satu kali saja. Artinya keluaran sisi tepi sudah berbentuk \textit{event} tervalidasi --- bukan deteksi mentah --- yang layak menjadi muatan aliran ke lapisan distribusi di sub-bab berikutnya.
...
\end{table}
\subsection{Visualisasi \textit{Dashboard} Analitik Waktu Nyata (\textit{Real-Time Dashboard})}
...
\caption{Antarmuka utama \textit{dashboard} Grafana yang memvisualisasikan data operasional riil selama 10 jam. Sistem merender Indikator Kinerja Utama (KPI) serta dinamika pelanggaran per 15 menit berdasarkan ekstraksi CCTV.}
...
\begin{table}[htbp]
\centering
\caption{Daftar segmentasi panel pada \textit{dashboard} analitik berdasarkan rancangan fungsional sistem.}
...
% ============================================================
\subsection{Analisis Wawasan Lapangan Berdasarkan Visualisasi \textit{Dashboard}}
