...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{gambar/aug_noise.jpg}
...
\textbf{Tahap 0 --- Deteksi mentah YOLO.} Tahap dasar menghasilkan 9.544 \textit{bounding box} dari ketiga kelas atau rata-rata 3,18 deteksi per \textit{frame}, sejalan dengan rata-rata 3,36 objek aktif pada Tabel~\ref{tab:bytetrack-basic-stats}. Komposisi didominasi \texttt{MOTORCYCLE} karena badan motor terlihat utuh di hampir setiap \textit{frame}, sedangkan \texttt{DRIVER\_HELMET} dan \texttt{DRIVER\_NO\_HELMET} muncul saat kepala pengendara tidak terhalang penumpang atau kaca depan. Angka mentah ini sekaligus menjadi tolok ukur beban inferensi YOLOv8 dan pembagi untuk seluruh persentase di kolom \textit{\% Sisa}. Tanpa filter di tahap berikutnya, jumlah ini setara dengan ratusan ribu baris log per jam jika diekstrapolasi ke operasi penuh kamera gerbang.

\textbf{Tahap 1 --- Seleksi kelas target.} Filter \textit{class-conditional} memangkas 7.942 deteksi non-target dan menyisakan 1.602 kandidat \texttt{DRIVER\_NO\_HELMET} atau 16,8\% dari mentah. Reduksi 83,2\% ini dicapai tanpa biaya komputasi berarti karena hanya membaca label keluaran YOLO, jauh lebih murah dibandingkan operasi geometris pada tahap-tahap berikutnya. Tujuan tahap ini bukan menyensor data lain, melainkan memfokuskan jalur pelanggaran; \texttt{MOTORCYCLE} tetap dilestarikan di buffer pelacakan untuk dipakai sebagai pasangan IoA di Tahap 3, sedangkan \texttt{DRIVER\_HELMET} memang tidak masuk jalur \textit{event}. Posisi tahap ini di paling depan dirancang sengaja agar tahap mahal seperti IoA dan konfirmasi temporal hanya menghitung kandidat yang berpotensi menjadi pelanggaran.

\textbf{Tahap 2 --- Penyematan \textit{Track ID}.} Jumlah kandidat tidak berubah pada tahap ini (1.602 masuk, 1.602 keluar), namun keluaran ByteTrack mengubah karakter datanya. Tiap kandidat kini membawa identitas persisten, sehingga dua tahap berikutnya dapat berbicara tentang ``objek yang sama'' pada \textit{frame} yang berbeda dan bukan sekadar kotak deteksi anonim. Beban tambahan di sini terukur 1,62 milidetik per \textit{frame} pada Tabel~\ref{tab:latency-decomposition} atau hanya 3,1\% dari total latensi sistem, harga yang sebanding dengan fungsinya sebagai fondasi konfirmasi N-\textit{frame} dan \textit{dedup}. Ada satu catatan yang perlu disampaikan terbuka: \texttt{Track ID} di sini bersifat identitas sesi pelacakan, bukan identitas pelanggar fisik permanen, sehingga pengendara yang masuk-keluar area pantau lebih dari 60 \textit{frame} akan memperoleh ID baru \cite{zhang2022bytetrack_eccv}.

\textbf{Tahap 3 --- Filter spasial IoA.} Tahap ini menjadi penyaring paling agresif dengan membuang 1.567 dari 1.602 kandidat atau 97,8\% dalam satu langkah. IoA yang dihitung mengikuti definisi pada Bab~\ref{chap:metodologi} membandingkan luas irisan \textit{bounding box} \texttt{DRIVER\_NO\_HELMET} terhadap luasnya sendiri, lalu menerima kandidat hanya bila skornya $\geq$0{,}3 terhadap salah satu \texttt{MOTORCYCLE} aktif. Mayoritas kandidat yang gugur berasal dari tiga sumber yang konsisten dengan kondisi gerbang Itera: pejalan kaki di sisi trotoar yang sesaat terklasifikasi sebagai \texttt{DRIVER\_NO\_HELMET}, deteksi tipuan (\textit{false positive}) di tepi citra yang tidak mempunyai pasangan motor, dan pengendara yang motornya sempat terhalang kendaraan lain sehingga relasi spasialnya tidak terbentuk. Pemilihan ambang 0{,}3 merupakan kompromi yang berhasil: nilai lebih rendah (0{,}1) akan meloloskan asosiasi palsu pada keramaian, sedangkan nilai lebih tinggi (0{,}5) akan menggugurkan kasus motor parsial yang sebenarnya valid. IoA dipilih ketimbang IoU karena \textit{bounding box} pengendara jauh lebih kecil daripada motor, sehingga IoU menghasilkan skor rendah meski pengendara jelas berada di atas motor \cite{aboah2023helmet}.

\textbf{Tahap 4 --- Konfirmasi N-\textit{frame}.} Filter temporal membuang 10 dari 35 kandidat yang lolos IoA, atau 28,6\% dari jumlah yang masuk. Mekanismenya sederhana: \textit{streak} hanya naik bila status \texttt{DRIVER\_NO\_HELMET} bertahan pada \texttt{Track ID} yang sama di tiga \textit{frame} berturut-turut, sehingga deteksi yang berkedip akibat bayangan, pantulan kaca, atau klasifikasi salah sesaat tidak meneruskan langkah berikutnya. Filter ini menambah jeda pelaporan sekitar 150 milidetik (tiga \textit{frame} pada 18,9 FPS), nilai yang tetap nyaman untuk pemantauan \textit{near real-time} karena masih jauh di bawah ambang persepsi operator. Mekanisme \textit{patience-based decay} dengan lima \textit{frame} masa tenggang melengkapi tahap ini agar \textit{streak} tidak gugur hanya karena oklusi sesaat oleh kendaraan lain --- sebuah kompromi praktis antara ketegasan temporal dan ketahanan terhadap gangguan visual. Reduksi 28,6\% di sini relatif kecil dibanding Tahap 3, dan justru itu yang diharapkan: kandidat yang sudah lolos IoA umumnya merupakan pengendara nyata, jadi tahap temporal hanya memangkas sisa-sisa \textit{flicker} yang masih terbawa.

\textbf{Tahap 5 --- \textit{Dedup} per-\textit{track}.} Tahap penutup memadatkan 25 konfirmasi tingkat-\textit{frame} menjadi 2 \textit{event} unik dengan membuang 23 duplikat atau 92,0\% dari masukan. Logikanya, satu \texttt{Track ID} yang sudah melewati konfirmasi N-\textit{frame} bisa terus terkonfirmasi sepanjang pengendara terlihat di kamera, sehingga tanpa \textit{dedup}, satu orang yang melintas selama 5 detik pada 15 FPS dapat tercatat hingga 75 kali. \textit{Dedup} memakai \texttt{Track ID} sebagai kunci dan memilih konfirmasi pertama sebagai representasi \textit{event}, lalu menahan ID tersebut hingga sesi pelacakan berakhir. Implikasinya jelas pada beban hilir: \textit{data mart} hanya menerima entri yang berkorelasi dengan pengendara unik, dan \textit{dashboard} menampilkan jumlah pelanggar bukan jumlah \textit{frame}. Keterbatasan tahap ini terikat pada keterbatasan Tahap 2: ketika ByteTrack memberi ID baru karena objek menghilang lebih dari 60 \textit{frame}, pengendara yang sama dapat tercatat dua kali --- isu yang dimasukkan ke agenda \textit{re-identification} antarkamera pada Bab~\ref{chap:kesimpulan}.

Akumulasi enam tahap di atas membentuk corong asimetris dengan dua kantong reduksi utama: Tahap 1 yang membuang 83,2\% kandidat secara murah lewat label, dan Tahap 3 yang membuang 97,8\% sisa kandidat lewat geometri. Tahap 2, 4, dan 5 berperan sebagai pemastian kualitas: Tahap 2 menyiapkan identitas, Tahap 4 menyaring kebisingan temporal, Tahap 5 mencegah duplikasi pencatatan. Komposisi ini cocok dengan kondisi lapangan gerbang Itera --- lalu lintas didominasi pengendara berhelm sehingga kelas target memang minoritas, dan pejalan kaki di sekitar gerbang menyumbang banyak kandidat semu yang baru bisa difilter dengan asosiasi spasial. Berdasarkan struktur reduksi ini, sistem dapat dianggap berhasil mengubah keluaran sisi tepi dari \textit{stream} deteksi mentah menjadi rangkaian \textit{event} tervalidasi yang siap dialirkan ke lapisan distribusi pada sub-bab berikutnya.
...
\subsection{Ringkasan Jawaban Rumusan Masalah 2}
\label{subsec:sintesis-rm2}

Tiga sub-bab di atas menghasilkan empat bukti yang saling melengkapi. Persistensi \texttt{Track ID} dan rangkaian filter spasial-temporal terbukti mengubah 9.544 deteksi mentah pada potongan uji 3.001 \textit{frame} menjadi 2 \textit{event} tervalidasi, sedangkan pada operasi penuh 10 jam (dilaporkan pada Sub-bab~\ref{subsec:analisis-data-lapangan}), kerangka filter yang sama menyaring 11.172 deteksi mentah menjadi 113 \textit{event}. Keluaran sisi tepi pada kedua skala memang sudah berbentuk peristiwa yang layak dicatat, bukan deteksi kasar. Sisi tepi bekerja stabil pada rerata 18,9 FPS dengan latensi 53,06 milidetik per \textit{frame}, di atas kebutuhan operasional CCTV kampus dan masih dalam rentang \textit{near real-time}. Lapisan distribusi tidak menjadi titik macet: Kafka menyalurkan seluruh \textit{event} tanpa kegagalan dengan latensi median 2,65 milidetik dan kapasitas 11.485 pesan per detik, sedangkan Spark menuntaskan tiap \textit{micro-batch} dalam 1,7 sampai 3,8 detik---jauh di bawah \textit{trigger interval} 10 detik. Dengan keempat bukti tersebut, pertanyaan Rumusan Masalah ke-2 sudah memperoleh jawabannya: sistem mampu menyatukan deteksi, pelacakan, validasi spasial, validasi temporal, distribusi pesan, dan ETL aliran menjadi satu kesatuan \textit{pipeline} yang menghasilkan \textit{event stream} pelanggaran helm tervalidasi dari kamera sampai \textit{data mart}. Catatan yang perlu disampaikan terbuka: keempat bukti di atas berasal dari pengukuran komponen, bukan dari pencocokan satu-satu \textit{event pipeline} dengan anotasi manual; validasi \textit{end-to-end} berbasis \textit{ground truth} manusia tetap menjadi langkah pelengkap yang dimasukkan ke agenda pengembangan lanjutan pada Bab~\ref{chap:kesimpulan}.
% ------------------------------------------------------------
\section{\textit{Data Mart} dan Visualisasi Analitik}
\label{sec:data-mart}

Bab 1 menanyakan bagaimana skema \textit{data mart} berbasis \textit{star schema} dan \textit{dashboard} analitik dirancang untuk mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan (\textit{Decision Support System}) yang memetakan pola spasiotemporal pelanggaran di Itera. Sub-bab ini menjawab pertanyaan tersebut dengan urutan kerja yang paralel dengan Tahap 4 metodologi pada Bab 3, yaitu implementasi \textit{star schema} di PostgreSQL, visualisasi \textit{dashboard} Grafana, dan analisis wawasan lapangan atas rekaman operasional 10 jam di gerbang utama Itera. Pembahasan dibagi menjadi empat bagian. Tiga bagian pertama mengikuti urutan kerja lapisan hilir, dan bagian terakhir merangkai keempat hasil menjadi jawaban atas Rumusan Masalah ke-3.
% ------------------------------------------------------------
...
Tiga tabel dimensi bekerja sebagai pengaya konteks tabel fakta dengan pembagian peran yang tegas. Dimensi \texttt{dim\_camera} memuat identitas dan lokasi geospasial sensor, sehingga setiap insiden dapat dirunut ke titik pengamatan tertentu tanpa menyimpan kolom lokasi yang sama berulang di tabel fakta. Dimensi \texttt{dim\_violation\_type} menampung kode dan deskripsi jenis pelanggaran; struktur ini disiapkan untuk perluasan ke pelanggaran lain---seperti berbonceng tiga atau penggunaan ponsel saat berkendara---tanpa perubahan skema. Dimensi \texttt{dim\_time} merinci atribut tanggal, jam, kuartal 15 menit, dan periode (pagi, siang, sore) yang menjadi fondasi seluruh agregasi temporal pada \textit{dashboard}. Tabel fakta \texttt{fact\_violations} sendiri menyisakan kolom metrik (probabilitas deteksi, latensi pemrosesan) dan \textit{foreign key} ke ketiga dimensi, sehingga ukurannya tetap ramping meski jumlah baris bertambah dari hari ke hari.

Skema bintang dipilih ketimbang skema kepingan salju (\textit{snowflake}) atau tabel tunggal terdenormalisasi karena tiga alasan praktis. Pertama, kueri analitik di Grafana mengandalkan SQL standar dengan satu \textit{level join} antara fakta dan dimensi, sehingga panel \textit{time-series} dapat dibangun tanpa CTE bertingkat. Kedua, denormalisasi pada level dimensi membuat atribut deskriptif (misalnya periode harian dan kode pelanggaran) terbaca tanpa biaya kueri tambahan, sesuai praktik pemodelan dimensional yang umum pada gudang data analitik. Ketiga, ekspansi ke jenis pelanggaran lain atau kamera tambahan hanya memerlukan baris baru di dimensi, bukan perubahan struktur tabel fakta. Konfigurasi ini menyiapkan landasan agar lapisan visualisasi pada Sub-bab~\ref{subsec:visualisasi-dashboard} bekerja dengan kueri yang ringkas dan stabil.

Berbeda dengan pengujian \textit{pipeline} pada Sub-bab~\ref{subsec:persistensi-filter} yang memakai potongan 3.001 \textit{frame}, bagian ini melaporkan operasi sistem pada rekaman penuh. Sumber data: kamera \texttt{gate\_utama\_01} di gerbang utama Itera, direkam selama 10 jam pada hari kerja pukul 07.00--17.00 WIB. Pada rentang waktu tersebut, \textit{pipeline} memproses 11.172 deteksi mentah dari ketiga kelas dan menyalurkannya lewat rangkaian filter spasial-temporal yang sama dengan pengujian sebelumnya. Setelah seleksi kelas, filter IoA, konfirmasi N-\textit{frame}, dan \textit{dedup} per-\textit{track} berjalan, \textit{data mart} menerima 113 insiden pelanggaran berkendara tanpa helm secara persisten. Tiap baris terhubung ke dimensi waktu (jam dan periode pagi, siang, sore), dimensi kamera, dan dimensi jenis pelanggaran.
...
\toprule
\textbf{Waktu} & \textbf{Kamera} & \textbf{Jenis Pelanggaran} & \textbf{Probabilitas} & \textbf{Latensi (ms)} \\
\midrule
...
% ------------------------------------------------------------
\subsection{Ringkasan Jawaban Rumusan Masalah 3}
\label{subsec:sintesis-rm3}

Tiga sub-bab di atas menghasilkan empat bukti yang saling melengkapi. Skema bintang dengan tabel fakta \texttt{fact\_violations} dan tiga tabel dimensi (\texttt{dim\_camera}, \texttt{dim\_violation\_type}, \texttt{dim\_time}) terbukti mampu memuat 113 insiden pelanggaran dari rekaman 10 jam operasional di gerbang utama Itera secara persisten dan siap untuk kueri agregasi. \textit{Dashboard} Grafana memvisualisasikan keempat sudut pandang yang dibutuhkan pengelola: tren agregat harian, distribusi 15 menit, akumulasi jam, dan matriks temporal kepadatan; keempatnya dirakit dari sumber data yang sama tanpa duplikasi. Analisis wawasan lapangan menonjolkan pola spasiotemporal nyata, yaitu distribusi bimodal pagi--sore dengan puncak absolut pukul 14.45--15.15 WIB yang berkorelasi dengan transisi jadwal kelas dan kondisi pasca-hujan. Stabilitas \textit{Avg Confidence} 79,9\% dan \textit{Avg Latency} 49 milidetik selama lonjakan beban memperlihatkan \textit{dashboard} masih dapat dipercaya saat data mengalir deras. Dengan keempat bukti tersebut, pertanyaan Rumusan Masalah ke-3 sudah memperoleh jawabannya: skema \textit{data mart} berbasis \textit{star schema} dan \textit{dashboard} analitik Grafana berhasil mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan yang memetakan pola spasiotemporal pelanggaran di kampus Itera secara terukur dan dapat ditindaklanjuti.
