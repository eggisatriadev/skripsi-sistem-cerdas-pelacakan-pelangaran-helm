...
Analisis eksploratif dilakukan pada dataset mentah hasil tangkapan CCTV. Dataset memuat 390 citra unik berukuran median $1280 \times 720$ piksel dan 2.369 anotasi objek (rata-rata 6,1 anotasi per citra). Tabel~\ref{tab:distribusi-objek} merinci distribusi anotasi setiap objek.
...
Tabel~\ref{tab:distribusi-objek} memperlihatkan proporsi antarobjek yang relatif seimbang. Anotasi \texttt{MOTORCYCLE} paling banyak (921), diikuti \texttt{DRIVER\_HELMET} (730) dan \texttt{DRIVER\_NO\_HELMET} (718) yang hampir setara. Komposisi seimbang seperti ini menekan risiko bias model \textit{deep learning} ke kelas mayoritas, kondisi yang pada kasus ekstrem memicu pengabaian kelas minoritas \cite{oksuz2020imbalance,johnson2019imbalance}. Selain distribusi kuantitatif, kualitas dataset juga ditinjau dari posisi kemunculan objek pada bidang pandang kamera. Gambar~\ref{fig:heatmap-per-class} menyajikan \textit{annotation heatmap} dengan gradasi biru hingga merah pekat sebagai indikator intensitas \textit{bounding box} per \textit{grid} citra.
...
Pada Gambar~\ref{fig:heatmap-per-class}, ketiga objek menunjukkan lokalisasi asimetris yang terpusat di area tengah hingga kanan atas citra. Pola ini mencerminkan lintasan utama kendaraan roda dua di gerbang masuk dan minimnya anotasi pada latar pasif (langit, trotoar, jalur berlawanan). Karena pola tersebut, ekstraksi \textit{Region of Interest} (ROI) mengikuti kontur jalan akan memangkas pemrosesan area latar yang tidak relevan dan memusatkan inferensi YOLOv8 pada zona bermakna. Sebaran \texttt{DRIVER\_NO\_HELMET} yang sedikit melebar ke kiri juga menyiratkan pola manuver pengendara tertentu. Penyempitan area komputasi berpotensi menaikkan \textit{precision} dan menekan \textit{false positive} pada inferensi waktu nyata.
